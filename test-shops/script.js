...
class AdminDashboard {
    ...
    async loadShops() {
        try {
            ...
            if (result.success) {
                this.shops = result.data.tenants || [];
                this.renderShops();
                this.updateStats();
            } else {
    ...
    updateStats() {
        ...
        const pending = this.shops.filter(shop => shop.status === 'pending').length;
        ...
        document.getElementById('installing-shops').textContent = pending + installing;
    ...
    openAdmin(subdomain) {
        const url = `http://${subdomain}.localhost/wp-admin`;
        window.open(url, '_blank');
    }
